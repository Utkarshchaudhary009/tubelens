...
- NEVER run a local dev server (`bun run dev`) for e2e. Real e2e happens only on the GitHub Action; the e2e bot starts the dev server on the PR and comments on the PR (see `AGENTS.E2E.md`).
...
Validation gates → mandatory review pass → genuine fixes → PR → up to 3 GitHub bot/review rounds → merge only on a clean merge gate → post-merge roadmap checkbox → post-merge production verification. Never let code, tests, `plans/`, and `docs/` drift.
...
One branch per phase → PR → Vercel preview → validation + e2e → up to 3 bot/review rounds → merge gate → merge → update roadmap checkbox on `main` → production health + relevant endpoint verification. Phase 1 additionally requires 7-day health/p95 validation before its phase can be declared complete.
...
4. Route e2e is owned by the GitHub Action e2e bot on the PR/preview (see `AGENTS.E2E.md`) — do not substitute local dev-server curl.
